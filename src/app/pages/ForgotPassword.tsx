...
export function ForgotPassword() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setStatusMessage(response.message || "Permintaan reset password berhasil dikirim.");
      if (response.resetUrl) {
        navigate(`/reset-password?token=${encodeURIComponent(new URL(response.resetUrl).searchParams.get('token') || '')}`);
        return;
      }
    } catch (error: any) {
      setErrorMessage(error.response?.data?.message || "Gagal mengirim instruksi reset password.");
    } finally {
      setIsLoading(false);
    }
  };
        ...
        <div className="bg-white rounded-2xl shadow-xl p-8">
          <div className="text-center mb-8">
            <Link to="/" className="inline-flex items-center space-x-2 mb-4">
              <img src="/logo.png" alt="Logo Grafenda" className="w-10 h-10 object-contain" />
            ...
            <p className="text-slate-600 mt-2">Masukkan email Anda untuk reset password.</p>
          </div>
