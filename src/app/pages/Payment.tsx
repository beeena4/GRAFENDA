...
export function Payment() {
  ...
  const [orderId, setOrderId] = useState<number | null>(location.state?.orderId || null);
  ...
  // 1 & 3. Sinkronkan nama variabel (totalActive) & gunakan Optional Chaining Anti-Crash
  const activeOrders = service?.totalActive ?? service?.seller_active_orders ?? 0;
  const maxOrders = service?.seller_max_orders ?? 5; // Fallback jika limit kosong
  ...
  const handleSubmit = async () => {
    // 1. HARD BLOCK: Cegat Langsung di Baris Paling Atas
    if (isCheckoutMode && isSellerFull) {
      alert('Mohon maaf, antrean seller ini sedang penuh. Pesanan otomatis ditolak.');
      setError('Antrean Seller Penuh. Silakan coba kembali nanti.');
      return;
    }
    ...
    try {
      if (isCheckoutMode) {
        ...
        const mappedMethod = paymentMethod === 'transfer' ? 'bank_transfer' : paymentMethod === 'va' ? 'virtual_account' : 'e_wallet';
        await paymentAPI.uploadPaymentProof({
          order_id: orderId,
          payment_method: mappedMethod,
        });

        // ALUR INSTAN: Langsung ubah status pesanan menjadi 'process' (Sedang Diproses)
      ...
      const backendMessage = err.response?.data?.message || err.message || 'Gagal memproses pesanan.';
      
      // 3. Tangkap Error 400 (atau 409) dari Backend dan paksa munculkan alert
    ...
    return (
      <div className="min-h-screen bg-slate-50 flex items-center justify-center">
        <div className="text-center">
          ...
          {/* 2. Handle Status Loading */}
          <p className="text-slate-600">Memuat ketersediaan seller...</p>
        </div>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-slate-50 py-8">
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <h1 className="text-3xl font-bold text-slate-800 mb-4">
          {isCheckoutMode ? 'Ajukan Pesanan' : 'Pembayaran'}
        </h1>
        <p className="max-w-2xl text-slate-600 mb-8">
          {isCheckoutMode ? 'Tuliskan catatan brief pesanan Anda untuk dikonfirmasi oleh freelancer.' : 'Lanjutkan pembayaran untuk pesanan Anda yang telah disetujui.'}
        </p>
        ...
        <div className="grid lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 space-y-6">
            <div className="bg-white rounded-2xl shadow-sm p-6">
              ...
              {isCheckoutMode ? (
                <div className="mt-6">
                  <label className="block text-sm font-medium text-slate-700 mb-2">Catatan / Brief untuk penjual</label>
                  <textarea
                    value={orderNotes}
          ...
          <div className="lg:col-span-1">
            <div className="bg-white rounded-2xl shadow-sm p-6 sticky top-24">
              ...
              <div className="space-y-4 mb-6">
                ...
                <div className="border-t border-slate-200 pt-4 space-y-3">
                  ...
                  <div className="border-t border-slate-200 pt-3 flex justify-between items-center">
                    ...
                    <span className="text-blue-600 font-bold text-xl">Rp {isCheckoutMode ? packagePrice.toLocaleString('id-ID') : totalAmount.toLocaleString('id-ID')}</span>
                  </div>
                </div>
              </div>
              ...
              <button
                ...
                disabled={!selectedPackage || submitting || (isCheckoutMode && isSellerFull)}
                className={`w-full flex justify-center items-center text-white py-3 rounded-xl transition-all duration-200 font-medium ${
                  selectedPackage && !submitting && !(isCheckoutMode && isSellerFull)
                    ? 'bg-gradient-to-r from-blue-600 to-purple-500 hover:shadow-xl hover:opacity-95 active:scale-95 cursor-pointer' 
                    : 'bg-slate-300 cursor-not-allowed'
                }`}
              >
                {submitting && <Loader className="w-5 h-5 animate-spin mr-2" />}
                {submitting ? 'Memproses...' : isCheckoutMode ? (isSellerFull ? 'Antrean Penuh' : 'Ajukan Pesanan') : 'Bayar Sekarang'}
              </button>
